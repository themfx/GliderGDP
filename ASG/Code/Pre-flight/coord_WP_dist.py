...
def get_coords(filename):
    """This function reads an input file in a standard APM/Mission Planner 2.0 format and exports them as a\
list of waypoints in a coordinate list with each element in format [latitude, longitude, altitude]."""
    coords=[]
...
"""subfunctions can now be called from top level."""
def haversin(theta):
...
def WP_dist(coords):
    """This function uses the Haversine Formula [1, accessed 2014, Nov 4th, 20:45] to compute distances between\
a series of waypoints. input 'coords' is a list with minimum two elements in format \
[latitude, longitude, altitude]. Output in form [distance(WP1,WP2), altitude(WP1)]. \
Altitude is assumed to have little affect and it is not included in calculation."""
    #source1: http://en.wikipedia.org/wiki/Haversine_formula
    
    

    #check if input has minimum of two elements, else return error.
    if len(coords)<2:
        print 'Input list does not contain enough elements.'
        return None
    out_dist=[]
    out_alt=[]
