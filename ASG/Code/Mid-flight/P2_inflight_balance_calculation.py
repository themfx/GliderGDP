...
float(z)
"""
def flight_balance(x,z,N,h_route,d_route):

    alpha=[]
    beta=[]

    for i in range(N-1):
        alpha.append(arraysum(d_route,i))
        beta.append(arraysum(d_route,i+1))

    for i in range(N-1):
        if alpha[i] < x < beta[i]:
            print ("You are at Stage %d: WP%d -> WP%d" %(i+1,i+1,i+2))
            distNextWP = beta[i]-x
            print ("Distance to WP%d is %.3f m" %(i+2,distNextWP))
            inflight_stage(z,h_route[i+1],distNextWP)
        elif x == beta[i] and x != beta[N-2]:
            print ("You are at WP%d" %(i+2))
            distNextWP = d_route[i+1]
            print ("Distance to WP%d is %.3f m" %(i+3,distNextWP))
            inflight_stage(z,h_route[i+2],distNextWP)
        elif x >= beta[N-2]:
            print ("You are beyond programmed flight path")
